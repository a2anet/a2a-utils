...
describe("A2ATools init", () => {
    test("default artifact settings", () => {
        ...
        expect((tools as any).artifactSettings.sendMessageCharacterLimit).toBe(50_000);
        expect((tools as any).artifactSettings.minimizedObjectStringLength).toBe(5_000);
        expect((tools as any).artifactSettings.viewArtifactCharacterLimit).toBe(50_000);
    });
    ...
    test("custom artifact settings", () => {
        ...
        expect((tools as any).artifactSettings.sendMessageCharacterLimit).toBe(100_000);
        expect((tools as any).artifactSettings.minimizedObjectStringLength).toBe(10_000);
        expect((tools as any).artifactSettings.viewArtifactCharacterLimit).toBe(75_000);
    });

    test("session reference", () => {
        const session = makeSession();
        const tools = new A2ATools(session);
        expect((tools as any).session).toBe(session);
    });
});
...
describe("getAgents", () => {
    test("returns dict", async () => {
        const tools = makeTools();
        spyOn((tools as any).session.agentManager, "getAgentsForLlm").mockResolvedValue({
    ...
    test("empty with init errors", async () => {
        const tools = makeTools();
        spyOn((tools as any).session.agentManager, "getAgentsForLlm").mockResolvedValue({});
        (tools as any).session.agentManager.initErrors = {
    ...
    test("error returns dict with error", async () => {
        const tools = makeTools();
        spyOn((tools as any).session.agentManager, "getAgentsForLlm").mockRejectedValue(new Error("boom"));
        const result = await tools.getAgents();
...
describe("getAgent", () => {
    test("returns agent details", async () => {
        const tools = makeTools();
        spyOn((tools as any).session.agentManager, "getAgentForLlm").mockResolvedValue({
            name: "Agent A",
    ...
    test("not found returns actionable error", async () => {
        const tools = makeTools();
        spyOn((tools as any).session.agentManager, "getAgentForLlm").mockResolvedValue(null);
        spyOn((tools as any).session.agentManager, "getAgents").mockResolvedValue({
    ...
    test("error returns dict with error", async () => {
        const tools = makeTools();
        spyOn((tools as any).session.agentManager, "getAgentForLlm").mockRejectedValue(new Error("kaboom"));
        const result = await tools.getAgent("agent-a");
        expect(result.error).toBe(true);
...
describe("sendMessage", () => {
    test("returns serialized task", async () => {
        ...
        spyOn((tools as any).session, "sendMessage").mockResolvedValue(task);
    ...
    test("returns serialized message", async () => {
        ...
        spyOn((tools as any).session, "sendMessage").mockResolvedValue(msg);
        const result = await tools.sendMessage("agent-a", "hello");
    ...
    test("agent not found error", async () => {
        const tools = makeTools();
        spyOn((tools as any).session, "sendMessage").mockRejectedValue(
    ...
    test("generic error", async () => {
        const tools = makeTools();
        spyOn((tools as any).session, "sendMessage").mockRejectedValue(new Error("network error"));
        const result = await tools.sendMessage("agent-a", "hello");
        expect(result.error).toBe(true);
...
describe("getTask", () => {
    test("returns serialized task", async () => {
        ...
        spyOn((tools as any).session, "getTask").mockResolvedValue(task);
    ...
    test("agent not found error", async () => {
        const tools = makeTools();
        spyOn((tools as any).session, "getTask").mockRejectedValue(new Error("Agent 'bad' not found"));
    ...
    test("timeout error", async () => {
        const tools = makeTools();
        spyOn((tools as any).session, "getTask").mockRejectedValue(
            new DOMException("The operation timed out.", "TimeoutError"),
        );
        const result = await tools.getTask("agent-a", "task-1");
        expect(result.error).toBe(true);
...
describe("viewTextArtifact", () => {
    test("returns serialized artifact", async () => {
        ...
        spyOn(tools as any, "getArtifact").mockResolvedValue(artifact);
    ...
    test("artifact not found error", async () => {
        const tools = makeTools();
        spyOn(tools as any, "getArtifact").mockRejectedValue(
...
describe("parseRows", () => {
    test("none", () => {
        expect((A2ATools as any).parseRows(null)).toBeNull();
    });

    test("all", () => {
        expect((A2ATools as any).parseRows("all")).toBe("all");
    });

    test("single int", () => {
        expect((A2ATools as any).parseRows("0")).toBe(0);
        expect((A2ATools as any).parseRows("5")).toBe(5);
    });

    test("range", () => {
        expect((A2ATools as any).parseRows("0-10")).toBe("0-10");
    });

    test("comma separated", () => {
        expect((A2ATools as any).parseRows("0,2,5")).toEqual([0, 2, 5]);
    });
    ...
    test("whitespace stripped", () => {
        expect((A2ATools as any).parseRows("  3  ")).toBe(3);
    });

    test("invalid comma separated", () => {
        expect(() => (A2ATools as any).parseRows("a,b,c")).toThrow("integers");
    });

    test("invalid string", () => {
        expect(() => (A2ATools as any).parseRows("abc")).toThrow("Invalid rows format");
    });
});

describe("parseColumns", () => {
    test("none", () => {
        expect((A2ATools as any).parseColumns(null)).toBeNull();
    });

    test("all", () => {
        expect((A2ATools as any).parseColumns("all")).toBe("all");
    });

    test("single column", () => {
        expect((A2ATools as any).parseColumns("name")).toBe("name");
    });

    test("comma separated", () => {
        expect((A2ATools as any).parseColumns("name,age")).toEqual(["name", "age"]);
    });

    test("comma separated with spaces", () => {
        expect((A2ATools as any).parseColumns("name, age, email")).toEqual(["name", "age", "email"]);
    });
